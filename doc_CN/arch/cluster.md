...
实施在 `feature/cluser2605` 分支上,跨多次提交。**完整 v1 决策回溯见 `docs/superpowers/specs/2026-05-12-cluster-design.md` §7;实施步骤见 `docs/superpowers/plans/2026-05-12-cluster-implementation.md`**。

- [x] 阶段 1：成员管理 + `/api/cluster/nodes`(Membership + Session 重注册 A1)
- [x] 阶段 2：流位置注册表 + `/api/cluster/streams`(StreamRegistry + 环回标记 Q2)
- [x] 阶段 3：跨节点拉流 relay(OnSubscribe + buildPullURL + cluster-relay marker + AddOnStreamRemoved 主动 Stop pull-proxy)
- [x] 阶段 4：录制 API 路由 + 删除 apiroute(**路线 B**:cluster 写 `APIRoute.Nodes` config + ClusterPlugin 实现 `m7s.RedirectAdvisorV2`;不引入新 `StreamRouter` 接口。`plugin/apiroute/` 已删)
- [x] 阶段 5：跨节点回看 + 共享存储 docs(mp4_streams 加 NodeID 列 + `/download` 跨节点 302 兜底 + SQLite warn-only P4)
- [x] 阶段 6：负载上报 + LB 建议(LoadReporter TickTask + `/api/cluster/lb-suggest`,排序按 streams → goroutines → NodeID)
- [x] 端到端:`example/cluster-e2e/` docker-compose(3 m7s + consul + postgres + minio)+ smoke.sh(10 个场景中 5 个自动 + 5 个手动)

### 决策回溯(本 v1 最终态)

| 编号 | 决策 | 含义 |
|---|---|---|
| D1 | 删除 `plugin/apiroute/` | cluster 完全替代,提交 `ecb2f260` |
| D2 (路线 B) | cluster 写 APIRoute.Nodes config + 实现 RedirectAdvisorV2;不引入 StreamRouter 接口 | brainstorming 时从 D2=A 调整。理由:m7s core 不存在可插 `StreamRouter` 接口,加新接口需改 core 50+ 行;路线 B 零 core 改动,只是 cluster 周期同步 peer 列表给静态 APIRoute config |
| D3 | 用 Consul,不用 etcd | 服务发现 / KV 后端 |
| A1 | session 失效后所有本地流位置 re-Acquire | `StreamRegistry.rebindAll` 实现 |
| Q2 | cluster-relay 标记走 `PullProxyConfig.Description` 前缀 | 不加结构字段。`ClusterRelayDescPrefix = "cluster-relay:"` |
| P4 | cluster + SQLite 仅 Warn,不拒启 | 允许"试用",生产文档要求 PostgreSQL |
| **新** | Origin 失联 → 主动 Stop 本地 cluster-relay pull-proxy | `StreamRegistry.AddOnStreamRemoved` + Relay 订阅 |
| **新** | first-write-wins 失败 → 强制 Stop publisher 带 `ErrStreamPathTaken` | `StreamRegistry.SetOnStopPublisher` |
| **新** | 集群内 relay 无 auth,部署级"内网信任面"约束 | 写进 README + 部署文档 |
| **新** | e2e 走 docker-compose,10 个 smoke 场景 | 不用 testcontainers-go |
| **新** | LoadReporter 仅采 streams/goroutines | CPU/带宽/subscribers v2 再加 |

### 已知未完成

- `go test -race -tags cluster` 因 **upstream `github.com/langhuihui/gotask` 内部 EventLoop 字段无锁** 而 fail,本仓代码层面无 race。详见根目录 `CLAUDE.md` "Known Issues" 段。
- `smoke.sh` 10 场景中 5 个(4/5/6/7/9)需在真实 docker stack 上手动逐一 curl 验证,等真实环境跑过再决定能否补 assertion 自动化。
- `example/cluster-e2e/` 整套 docker stack 本环境**未真跑**,只验过 yaml / Dockerfile / bash 语法。
